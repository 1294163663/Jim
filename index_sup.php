...
<div style="text-align: center">
    <h1 >中学生信息管理系统</h1>
    <a href="xls.php?t=3" >学生信息导出</a>
    <a href="xls.php?t=1" >老师信息导出</a>
</div>
<div style="text-align: center">

    <a href="index_sup.php?myid=root" >主页</a>
    <a href="add.php?myid=root" >添加用户</a>
...
//    链接数据库
    ...
    if( $pagex == 1 ){
        ...
        $page_stringx .= '<a href=?myid='.$_GET['myid'].'&pagex=1>第一页</a>|<a href=?myid='.$_GET['myid'].'&pagex='.($pagex-1).'>上一页</a>|';
    }
    if( ($pagex == $pagemountx) || ($pagemountx == 0) ){
        $page_stringx .= '下一页|尾页';
    }
    else{
        $page_stringx .= '<a href=?myid='.$_GET['myid'].'&pagex='.($pagex+1).'>下一页</a>|<a href=?myid='.$_GET['myid'].'&pagex='.$pagemountx.'>尾页</a>';
    }
...
<table width="100%" border="1" >
    ...
    if( $paget == 1 ){
        ...
        $page_stringt .= '<a href=?myid='.$_GET['myid'].'&paget=1>第一页</a>|<a href=?myid='.$_GET['myid'].'&paget='.($paget-1).'>上一页</a>|';
    }
    if( ($paget == $pagemountt) || ($pagemountt == 0) ){
        $page_stringt .= '下一页|尾页';
    }
    else{
        $page_stringt .= '<a href=?myid='.$_GET['myid'].'&paget='.($paget+1).'>下一页</a>|<a href=?myid='.$_GET['myid'].'&paget='.$pagemountt.'>尾页</a>';
    }
    ...
    foreach ($pdo->query($sql)  as  $val){
        ...
        echo "<tr>
               ...
               <td>$val[identity]</td>
               <td>
                   <a href='edit_tr.php?id_tr=$val[id_tr]&myid=root&t=1'>查看</a>
              </td>
               </tr>";
    }
    ?>
...
<?php
    ...
    $page_stringz .= '<a href=?myid=' . $_GET['myid'] . '&pagez=1>第一页</a>|<a href=?myid=' . $_GET['myid'] . '&pagez=' . ($pagez - 1) . '>上一页</a>|';
    }
    if (($pagez == $pagemountz) || ($pagemountz == 0)) {
    $page_stringz .= '下一页|尾页';
    } else {
    $page_stringz .= '<a href=?myid=' . $_GET['myid'] . '&pagez=' . ($pagez + 1) . '>下一页</a>|<a href=?myid=' . $_GET['myid'] . '&pagez=' . $pagemountz . '>尾页</a>';
    }
    ...
    echo "<tr>
        ...
        <td>$val[identity]</td>
        <td>
            <a href='edit_tr.php?id_tr=$val[id_tr]&myid=$_GET[myid]&t=3'>查看</a>
        </td>
    </tr>";
    }
