...
switch ($_GET['action']) {
    ...
    case 'Del':   //删除信息
        ...
        $sre = $pdo->exec($sql);
        if ($sre) {
            echo "<script>alert('删除成功');window.location='index_sup.php?myid=root';</script>";

        } else {
            echo "<script>alert('删除失败');window.location='index_sup.php?myid=root';</script>";
        }
        break;
    ...
    case 'Edt':        //修改信息
        ...
        if ($sre) {
            if ($_GET['myid'] == 'root') {
                echo "<script>alert('修改成功');window.location='index_sup.php?myid=$_GET[myid]';</script>";
            } else {
                echo "<script>alert('修改成功');window.location='index.php?myid=$_GET[myid]';</script>";
            }
    ...
    case  'Reg':
        ...
        if ($sre) {
            if (@$_GET['myid'] == 'root') {
                echo "<script>alert('添加成功');window.location='index_sup.php?myid=root';</script>";
            } else {
                $sre = $pdo->exec($sql);
                if ($sre) {
                    if (@$_GET['myid'] == 'root') {
                        ...
                        $sql = "SELECT id FROM student WHERE studyid='$id_st'";
                        $stm = $pdo->query($sql);
                        ...
                        echo "<script>alert('注册成功');window.location='index.php?myid=$arr[id]  ';</script>";
                    }
    ...
    //登录账户
       case 'Log':
        ...
        if ($id_st == 'root' && $pw == 'root') {
            ...
            unset($_SESSION['int']);
            $_SESSION['uid'] = 'root';
            header("Location:index_sup.php?myid=root");
        ...
        if ($pw == $arr['password']) {
            ...
            unset($_SESSION['int']);
            $_SESSION['uid'] = $arr['id'];
            header("Location:index.php?myid=$arr[id]");
        } else {
    ...
    case 'Tra':
        ...
        $sre=$pdo->exec($sql);
        if($sre) {
            echo "<script>alert('等待审核');window.location='index.php?myid=$_GET[id]';</script>";
        }else {
            print_r($pdo->errorInfo());
            echo "<script>alert('修改失败');window.history.back();</script>";
        }
    ...
    case 'Etr':
        ...
        if ($sre) {
            if ($_GET['myid']=='root'){
                echo "<script>alert('审核成功');window.location='index_sup.php?myid=$_GET[myid]';</script>";
            }else{
                echo "<script>alert('审核成功');window.location='index.php?myid=$_GET[myid]';</script>";
            }
